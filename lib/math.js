...
function composeMat2d(out, position, scale, rotation) {
    var sx = scale[0], sy = scale[1], c = Math.cos(rotation), s = Math.sin(rotation);
    out[0] = c * sx;
    out[1] = s * sx;
    out[2] = -s * sy;
    out[3] = c * sy;
...
var extractScale_VEC2_0 = gl_matrix_1.vec2.create();
...
function getAngleFromPoint(out) {
    return Math.atan2(out[1], out[0]) - exports.HALF_PI;
}
exports.getAngleFromPoint = getAngleFromPoint;
var getTangentAngle_VEC2_0 = gl_matrix_1.vec2.create();
function getTangentAngle(vec) {
    var tmp = gl_matrix_1.vec2.copy(getTangentAngle_VEC2_0, vec), tmpX = tmp[0];
    tmp[0] = tmp[1];
    tmp[1] = tmpX;
    return getAngleFromPoint(tmp);
}
exports.getTangentAngle = getTangentAngle;
var getAngleBetweenPoints_VEC2_0 = gl_matrix_1.vec2.create();
function getAngleBetweenPoints(a, b) {
    return getAngleFromPoint(gl_matrix_1.vec2.sub(getAngleBetweenPoints_VEC2_0, b, a));
}
...
function projectPointOnAxis(out, point, axis) {
    var squaredLength = gl_matrix_1.vec2.squaredLength(axis), dotProduct = gl_matrix_1.vec2.dot(point, axis);
