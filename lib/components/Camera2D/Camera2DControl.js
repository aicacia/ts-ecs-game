...
var VEC2_0 = gl_matrix_1.vec2.create(),
  VEC2_1 = gl_matrix_1.vec2.create(),
  VEC2_2 = gl_matrix_1.vec2.create(),
  ZERO = gl_matrix_1.vec2.fromValues(0, 0),
  MIN_SCALE = gl_matrix_1.vec2.fromValues(1, 1);
var Camera2DControl = /** @class */ (function (_super) {
  ...
  Camera2DControl.prototype.onUpdate = function () {
    if (this.enabled) {
      var input = this.getRequiredPlugin(plugins_1.Input),
        transform = TransformComponent_1.TransformComponent.getRequiredTransform(
          this.getRequiredEntity()
        ),
        scale = transform.getLocalScale2(VEC2_1),
        camera = this.getRequiredComponent(Camera2D_1.Camera2D),
        worldMouse = camera.toRelative(
          VEC2_0,
          gl_matrix_1.vec2.set(
            VEC2_0,
      ...
      var mouseWheel = input.getValue("mouseWheel"),
        zoomSpeed = gl_matrix_1.vec2.set(
          VEC2_2,
          this.zoomSpeed,
          this.zoomSpeed
        );
  ...
  Camera2DControl.requiredComponents = [
    [Transform2D_1.Transform2D, Transform3D_1.Transform3D],
  ];
