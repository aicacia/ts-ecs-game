...
var Input = /** @class */ (function (_super) {
    ...
    function Input() {
        ...
        _this.addAxis(new InputAxis_1.InputAxis("horizontal-keys", "ArrowLeft", "ArrowRight"), new InputAxis_1.InputAxis("vertical-keys", "ArrowDown", "ArrowUp"));
        return _this;
    }
    ...
    Input.prototype.addAxes = function (axes) {
        var _this = this;
    ...
    Input.prototype.removeAxes = function (axes) {
        var _this = this;
    ...
    Input.prototype.addInputHandlers = function (inputHandlers) {
        var _this = this;
    ...
    Input.prototype.removeInputHandlers = function (inputHandlers) {
        var _this = this;
        inputHandlers.forEach(function (inputHandler) {
    ...
    Input.prototype.addEventListeners = function (eventListeners) {
        var _this = this;
    ...
    Input.prototype.removeEventListeners = function (eventListeners) {
        var _this = this;
        eventListeners.forEach(function (eventListener) {
    ...
    Input.prototype.onAfterUpdate = function () {
        var time = this.getRequiredPlugin(Time_1.Time);
        this.eventListeners.forEach(function (eventListener) {
            return eventListener.onAfterUpdate(time);
        });
        this.inputHandlers.forEach(function (inputHandler) {
    ...
    Input.prototype.toJSON = function () {
        return tslib_1.__assign(tslib_1.__assign({}, _super.prototype.toJSON.call(this)), { buttons: Object.values(this.buttons).map(function (button) { return button.toJSON(); }), axes: Object.values(this.axes).map(function (axis) { return axis.toJSON(); }), inputHandlers: this.inputHandlers.map(function (inputHandler) {
                return inputHandler.toJSON();
            }) });
    };
    Input.prototype.fromJSON = function (json) {
        ...
        if (json_1.isJSONArray(json.buttons)) {
            json.buttons.forEach(function (json) {
                var buttonJSON = json, button = new InputButton_1.InputButton(buttonJSON.name).fromJSON(buttonJSON);
                _this.buttons[button.getName()] = button;
            });
        }
        if (json_1.isJSONArray(json.axes)) {
            this.addAxes(json.axes.map(function (json) {
                var axisJSON = json;
        ...
        if (json_1.isJSONArray(json.inputHandlers)) {
            this.addInputHandlers(json.inputHandlers.map(function (json) {
                return InputHandler_1.InputHandler.newFromJSON(json);
            }));
        }
        return this;
    };
